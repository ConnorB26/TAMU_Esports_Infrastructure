...
export async function findOne(discord_id: string): Promise<QOTDEntry> {
...
export async function update(discord_id: string, QOTDEntry: Partial<QOTDEntry>): Promise<QOTDEntry> {
    return withAxiosErrorHandling(() => axios.put<QOTDEntry>(`${baseURL}/${discord_id}`, QOTDEntry));
}

export async function remove(discord_id: string): Promise<void> {
